...
signal_poll= [(530,230),(810,230),(810,570),(530,570)]
signalTimer_poll = [(530,210),(810,210),(810,550),(530,550)]
...
vehicleCount= [(480,210),(880,210),(880,550),(480,550)]
        ...
        for i in range(0,noOfSignals):  # display signal and set timer according to current status: green, yellow, or red
            if(i==currentGreen):
                if(currentYellow==1):
                    ...
                    screen.blit(yellowSignal, signal_poll[i])
                else:
                    signals[i].signalText = signals[i].green
                    screen.blit(greenSignal, signal_poll[i])
            else:
                if(signals[i].red<=10):
                    signals[i].signalText = signals[i].red
                else:
                    signals[i].signalText = "---"
                screen.blit(redSignal, signal_poll[i])
        signalTexts = ["","","",""]

        # display signal timer
        for i in range(0,noOfSignals):
            signalTexts[i] = font.render(str(signals[i].signalText), True, white, black)
            screen.blit(signalTexts[i],signalTimer_poll[i])

        # display vehicle count
        for i in range(0,noOfSignals):
            displayText = vehicles[directionNumbers[i]]['crossed']
            vehicleCountTexts[i] = font.render(str(displayText), True, black, white)
            screen.blit(vehicleCountTexts[i],vehicleCount[i])

        # display time elapsed
        timeElapsedText = font.render(("Time Elapsed: "+str(timeElapsed)), True, black, white)
        screen.blit(timeElapsedText,timeElapsed)
